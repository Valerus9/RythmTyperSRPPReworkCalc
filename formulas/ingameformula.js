let ingameformula ={
    name: "In game formula",
    creator: "Valerus9",
    pp:{
        calculate(scoreData)
        {
            const acc = scoreData.accuracy / 100;
            let result =ingameformula.innerCalculate(scoreData).difficultyDensity;
    ...
    buildup:{
        calculate(scoreData)
        {
            result =ingameformula.innerCalculate(scoreData);
            return result;
        }
    },
    innerCalculate(scoreData) {
        ...
        for (let i = 0; i < scoreData.notes.length; ++i) {
            if (scoreData.notes[i].type == "tap") {
                ...
                if (KEYBOARDLAYOUT.includes(tempNote.key))
                ...
                if (KEYBOARDLAYOUT.includes(tempHoldNote.key))
