ppFormulas = {
    ...
    valerusReworkV2(scoreData) {
        ...
        for (let difficultyIndexer = 0; difficultyIndexer < difficultyObjects.length; ++difficultyIndexer) {
            ...
            difficultySum += calculatedDifficulty * alternatingNerf * distanceFactor * speedFactor * handOverwhelmingBuff * lengthBonus * repeatedPatternNerf[difficultyIndexer];
        }
        //console.log(consoleList);
        let difficultyDensity = difficultySum / drainTime;
        //if (difficultyDensity > 5) {
        //    difficultyDensity = 5 * Math.pow(difficultyDensity / 5, 0.37);
        //}
        difficultyDensity = Math.pow(difficultyDensity * 20, 0.9);
        return difficultyDensity * Math.pow(acc, 5);
    }
};